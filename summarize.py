...
if __name__ == "__main__":
    ...
    def bin(instructions, index, base="", bin_progress=0, progress_out_of=None):
        ...
        for b in c.d:
            (c.d[b], bin_progress) = bin(
                c.d[b], index + 1, base + b, bin_progress, progress_out_of
            )
    ...
    def draw_infobox(gui, o):
        ...
        if type(o) == Catalog:
            ...
            line = line + 1
            gui.window.addstr(
                line, infobox_x + 2, "%s" % hexlify(o.example), gui.gray(0.8)
            )
            line = line + 1
            ...
            line = line + 1
            gui.window.addstr(
                line, infobox_x + 2, "%-30s" % hexlify(o.raw), gui.gray(1)
            )
            line = line + 1
    ...
    while True:
        ...
        gui.window.addstr(33, 1, "j: down,     J: DOWN", gui.gray(0.4))
        gui.window.addstr(34, 1, "k: up,       K: UP", gui.gray(0.4))
        gui.window.addstr(35, 1, "l: expand    L: all", gui.gray(0.4))
        gui.window.addstr(36, 1, "h: collapse  H: all", gui.gray(0.4))
